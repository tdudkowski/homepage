...
tags: react, gatsby
description: "Komponenty, layout.js i komponenty layoutu. Budowa struktury strony. Komponent Link. Przekazywanie propsów i sterowanie wyświetlaniem elementów strony."
---
...
Czasem przy tym samym pliku layoutu wystarczy zastosować różne CSS-y, żeby uzyskać zupełnie różne układy strony. Pierwszą popularną prezentacją możliwości CSS była strona [CSS Zen Garden](http://www.csszengarden.com/). Na tym etapie wystarczy użycie pliku layout.css i importowanie go do pliku strony lub pliku layoutu. Trzeba tylko pamiętać, że atrybutem klasy w JS jest [className](https://developer.mozilla.org/en-US/docs/Web/API/Element/className), ponieważ class jest w JavaScripcie słowem zastrzeżonym.
